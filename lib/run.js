...
(function() {
  var OptionsParser, PackageJSONReader, Promise, ReadmeCreator, TemplateLoader, fs, mustache, path, program, requireAll, winston;
  ...
  module.exports = ReadmeCreator = (function() {
    function ReadmeCreator(options) {
      ...
      winston.debug("------------------------------------------------");
      this.pkg = new PackageJSONReader().read();
        ...
        resolve: (function(_this) {
          return function(Component) {
            return new Component(_this.options).run(_this.pkg).tap(function() {
              return winston.debug(Component.name + " done");
            });
          };
        })(this)
      }));
    ...
    ReadmeCreator.run = function(args, cb) {
      ...
      optionNames = ['filename', 'debug', 'silent', 'encoding', 'addDesc', 'modules', 'npmcdn', 'licenseFile', 'badges', 'branch', 'docFile', 'replaceReferences'];
      program.version(require('../package.json').version).usage('[options] <file>').option('-d, --debug', 'Debug mode').option('-s, --silent', 'Silent mode').option('--encoding <enc>', 'Encoding to use to read files ["utf-8"]').option('-a, --add-desc <text>', 'Text to add to the description [""]').option('-u, --add-usage <text>', 'Text to add to the usage section [""]').option('-m, --modules <modules>', 'List of support module types ' + '["CommonJS"]', list).option('-n, --npmcdn', 'Delivery by npmcdn.com').option('--license-file <file>', 'Name of the license file ["LICENSE.txt"]').option('-b, --badges <badges>', 'Badges to use ["npm-version,travis,coveralls,' + 'dependencies,devDependencies,gitter"]', list).option('--branch <branch>', 'Branch to use for the documentation ["master"]').option('--doc-file <file>', 'Main html file for the documentation ["index.html"]').option('--no-replace-references', 'No replacement of "../" in examples').option('--replace-references', 'Force replacement of "../" in examples').parse(args);
      ...
      if (program.enableReplaceReferences && program.disableReplaceReferences) {
        winston.warn('Set both --enable-replace-references as well as ' + '--disable-replace-refrences');
      }
